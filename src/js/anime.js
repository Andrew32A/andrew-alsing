  ...
  const easings = (() => {
    ...
    function elastic(t, p) {
      return t === 0 || t === 1
        ...
        : -Math.pow(2, 10 * (t - 1)) *
            Math.sin(
              ((t - 1 - (p / (Math.PI * 2.0)) * Math.asin(1)) * (Math.PI * 2)) /
                p
            );
    }
    ...
    function setAnimationsProgress(insTime) {
      ...
      if (transformsLength) {
        for (let id = 0; id < transformsLength; id++) {
          if (!transformString) {
            const t = "transform";
            transformString = getCSSValue(document.body, t)
              ? t
              : `-webkit-${t}`;
          }
          instance.animatables[id].target.style[transformString] =
            transforms[id].join(" ");
        }
      }
